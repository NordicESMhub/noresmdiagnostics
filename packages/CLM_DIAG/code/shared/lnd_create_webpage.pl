...
# get environment variables
  ...
  if ($runtype eq "model-obs") { $obsFlag = 1; }
  else			       { $set1DiffFlag = 1; }
  if    ($plot_type eq "ps")   { $sfx = ".png"; }
...
for $set (@setList)
{
    ...
    if(!@status[$set-1]) { &set_Inactive($set); }
    if( @status[$set-1]) { 
...
sub set1Page
{
	...
	if ($set1DiffFlag) { printf(fp_wp "  <TH ALIGN=LEFT><font color=maroon> vs Difference</font>\n");  }
	printf fp_wp "<TR>\n";
        undef @fileList;
        $ctr=0;
	...
	# read list of output variables for set1
		...
		if ( -e $file) { printf fp_wp "  <TH ALIGN=LEFT><A HREF=\"$href\">plot</a>\n"; }
...
sub set2Page
{
	...
	printf fp_wp "<TR>\n";
	printf fp_wp "<TR>\n";
	printf fp_wp "  <TH ALIGN=LEFT><font color=navy size=+1>Model Only</font>\n";
	printf fp_wp "<TR>\n";
        $inFile = $wkdir."/master_set".$set.".txt";
        close(fp_in);
...
sub set7Page()
{
	...
	printf fp_wp "<TR>\n";
	printf fp_wp "  <TH ALIGN=LEFT>RTM flow at station versus obs for world's 10 largest rivers (QCHANR)\n";
	...
	printf fp_wp "<TR>\n";
	printf fp_wp "  <TH ALIGN=LEFT><font color=red>VARIABLES</font>\n";
	printf fp_wp "<TR>\n";
	printf fp_wp "  <TH ALIGN=LEFT>QCHANR \n";
	printf fp_wp "  <TH ALIGN=LEFT>NativeUnits [m^3/s] \n";
	printf fp_wp "<TR>\n";
	printf fp_wp "  <TH ALIGN=LEFT>QCHOCNR \n";
	printf fp_wp "  <TH ALIGN=LEFT>NativeUnits [m^3/s] \n";
	printf fp_wp "<TR>\n";
}
sub writeRegion()
