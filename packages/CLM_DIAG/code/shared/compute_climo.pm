...
sub create_climo_atm
{
  ...
  $err = system("$ncclimoDir/ncclimo --no_stdin --clm_md=mth -m $mode -v $atmVars -a $djf_md --no_amwg_links -c $caseid -s $clim_fyr -e $clim_lyr -i $casedir -o $prefixDir"); die "ncclimo failed\n" if $err;
  chdir("$prefixDir");
