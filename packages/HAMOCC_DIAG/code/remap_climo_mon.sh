...
do
    ...
    eval $CDO -s remap,global_1,$wgt_file $climodir/$infile $climodir/$outfile >/dev/null 2>&1 &
    pid+=($!)
done
for ((m=0;m<=11;m++))
do
    wait ${pid[$m]}
    if [ $? -ne 0 ]; then
        echo "ERROR in remapping: $CDO -s remap,global_1,$wgt_file $climodir/$infile $climodir/$outfile"
        echo "*** EXITING THE SCRIPT ***"
        exit 1
    fi
done
