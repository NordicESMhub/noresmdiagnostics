...
# $PATHJLS  directory on dataproc where data is and output will be written
...
while ($YR <= $last_yr) 
  ...
  if ($SEAS_MEAN == jfm) then
    foreach month (01 02 03)
      ...
      set filenames = ($filenames ${PATHJLS}/${FILE_HEADER}${date}.nc)
    end
  endif
  ...
  if ($SEAS_MEAN == ann) then
    set filenames = (${filenames} ${PATHJLS}/${FILE_HEADER}${four_digit_year}*.nc)
  endif
